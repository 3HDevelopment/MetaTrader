...
while 1 != 24:
    ...
    rates = mt5.copy_rates_from_pos(instru, mt5.TIMEFRAME_H1, 0, 146)
    ...
    print(open, high, close, low, fast, slow)
    # logic 1
    if (slow < fast and any(slow < x < fast for x in [trend1, trend2, trend3, notrend])) or \
        ...
        while current == now:
            if slow > fast: ##close > open and  deleted##
                ...
                # if the symbol is unavailable in MarketWatch, add it
                    ...
                    "price": high,
                    "sl": (max(fast, high*0.9995)).round(5),  ## UNIQUE DECIMAL IS REQUIRED FOR EACH CURRENCY ##
                    "tp": (min(((high-fast)*1.75)+high, high*1.025)).round(5), ## UNIQUE DECIMAL IS REQUIRED FOR EACH CURRENCY ##
                    "deviation": deviation,
                    "magic": 234000,
            ...
            elif slow < fast: ## close < open and  deleted ##
                ...
                # if the symbol is unavailable in MarketWatch, add it
                    ...
                    "price": low,
                    "sl": (min(fast, low*1.0005)).round(5), ## UNIQUE DECIMAL IS REQUIRED FOR EACH CURRENCY ##
                    "tp": (max(low-((fast-low)*1.75), low*0.975)).round(5), ## UNIQUE DECIMAL IS REQUIRED FOR EACH CURRENCY ##
                    "deviation": deviation,
                    "magic": 234000,
                    "comment": "python script open",
                    "type_time": mt5.ORDER_TIME_GTC,
            ...
            print("Order sent. trading paused until next candle..")
            print(current, now)
            time.sleep(20)
